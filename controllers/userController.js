...
const userController = {
  ...
  getUser: (req, res) => {
    return User.findByPk(req.params.id, {
      ...
      let comments = user.Comments.length
      return res.render('profile', { user: JSON.parse(JSON.stringify(user)), comments })
    })
  },
  editUser: (req, res) => {
    if (req.user.id == req.params.id) {
      return User.findByPk(req.params.id).then(user => {
        console.log(user)
        return res.render('profileEdit', { user: JSON.parse(JSON.stringify(user)) })
      })
    } else {
  ...
  getTopUser: (req, res) => {
    ...
    return User.findAll({
      ...
      return res.render('topUser', { users: JSON.parse(JSON.stringify(users)) })
    })
  },
